        ...
        static void EnterField(ref Player player)
        {
            ...
            {    //다시금 선택하고 루프 돌 수 있도록 
                ...
                if (input == "1")
                {
                    ...
                    // 이 정보를 추가해주려고 보니 [player]가 어디있어? [메인]에서 정의되었어 (지금수준)에서는 함수로 받아주는 방법을 씀
                    // Main ~ EnterGame에서 player를 넘겨주고 -> 넘겨주려먼 함수 정의 부분에서 매개변수로 [Player player] 추가
                    // 위에서 받았으니까 그 아래에 있는 EngerField 함수에 매개변수에도 player 추가
                    // 그럼 다시 EngerField 정의된 부분에 player 추가  -> Fight 쓰려고 3번이나 길을 뚫어준셈
        ...
        public static void Main(string[] args)
        {
            while (true)                                      // 여기 조건식을 ( choice != ClassType.None ) 이런식으로 하고 아래 빠져나오는 분기문을 없애도 ok
            {
                ...
                if (choice != ClassType.None)
                {
                    ...
                    // 게임에 접속하는 메시지
                    // ref 를 쓰지 않으면 원본은 위에 [Player player] 담겨 있음. 복사본을 넣어준다
                    // 메인에서 정의한 player는 영향이 없어짐 그래서 계속 ref를 붙여서 전달해야 함 
                }
            }
